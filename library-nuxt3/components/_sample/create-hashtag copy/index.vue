  ...
  <div class="hashtag-area">
    <div class="hashtag-box">
      <span v-for="(item, index) in state.tagSet" :key="index" class="tag">
        ...
        <button 
          type="button"
          @click.prevent="handleOnDeleteTag(index)"
        >
          <BaseIcon
...
import { hashtagSetType } from '@/components/base/create-hashtag/HashtagMenu.vue'
...
export default defineComponent({
  ...
  setup(props) {
    const state = reactive<{ tagSet: hashtagSetType[], totalTagLength: number }>({
      tagSet: [],
      totalTagLength: 0
    })
    const handlenOnPushTag = (str:string) => {
      const filterTag = props.hashtagSet.filter(item => item.hashtagNm === str)
    ...
    return { state,  handlenOnPushTag, handleOnPopTag, handleOnDeleteTag, checkDoubleTag,   handlenOnSelectTag  }
  }
})
</script>
