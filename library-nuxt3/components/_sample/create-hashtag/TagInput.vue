...
import HashtagMenuBox, {
  hashtagSetType,
} from '@/components/hashtag/create-hashtag/HashtagMenuBox.vue'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
      ...
      if (evt.code === 'Comma' || evt.code === 'Enter') {
        if (state.inputValue !== '') {
          choiceTagsIndex.value !== null
            ? emit(
                'checkDoubleTag',
                filterTags.value[choiceTagsIndex.value].hashtagNm,
              )
    ...
    const handleOnChoiceTag = (type: 'increase' | 'decrease') => {
      ...
      switch (type) {
        case 'increase':
          ...
          // choiceTagsIndex.value =
          //   choiceTagsIndex.value < filterTags.value.length
          //     ? choiceTagsIndex.value + 1
          //     : choiceTagsIndex.value
        ...
        case 'decrease':
          ...
          console.log('decrease', choiceTagsIndex.value)
          break
        default:
    ...
    // Input Keyup Event
    onMounted(() => {
      inputRef.value!.addEventListener('keyup', (evt: KeyboardEvent) =>
        handleOnKeyUp(evt),
      )
